...
The T-Net is a network that estimates a affine transformation matrix. Given two affine spaces $A_1$ and $A_2$, an affine transformation, also called affinity, is a morphism between $A_1$ and $A_2$ such that the induced map $f_P: E_1 \shortrightarrow E_2$ between the Euclidean spaces $E_1$ and $E_2$ and with the point $P$ in $A_1$ is a linear map.
